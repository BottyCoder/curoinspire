...
// 4️⃣ **Serve Static Files** (like messagestatus.html, styles.css) from /public
...
app.post('/client-send-message', async (req, res) => {
  try {
    ...
    // Use UTC timestamp for storing in the database
    const timestampUTC = moment.utc().format();

    // Sanitize and replace "hi" / "hello" in the message
    ...
    // Store message data (insert or update) in Supabase
    const { data, error } = await supabase
      .from('messages_log')
      .upsert([{
        wa_id,
        original_wamid: wamid || null,
        tracking_code,
        client_guid,
    ...
    if (error) {
      console.error("❌ Error storing message data:", error.message);
      return res.status(500).json({ error: 'Failed to store message data' });
    }
...
// ----------------------------------------------------------------------------
// ADD THE GET-MESSAGE-STATUS ROUTE (for your webpage JS):
// ----------------------------------------------------------------------------
app.get('/get-message-status', async (req, res) => {
  ...
  try {
    ...
    // Return records to the frontend
    res.json({ messages: data });
  } catch (err) {
    console.error('Error in /get-message-status:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
